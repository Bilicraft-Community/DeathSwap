...
public class CountDownWatcher {
    int remains = DeathSwap.getInstance().getGame().getCountdown();
    int shorter = 5;
    ...
    public CountDownWatcher() {
        new BukkitRunnable() {
            @Override
            public void run() {
                Game game = DeathSwap.getInstance().getGame();
                ...
                if (game.getInGamePlayers().size() < game.getMinPlayers()) {
                    game.getInGamePlayers().forEach(p -> p.sendTitle(ChatColor.AQUA + "" + game.getInGamePlayers().size() + " " + ChatColor.WHITE + "/ " + ChatColor.AQUA + game.getMinPlayers(),
                            "正在等待更多玩家加入游戏....", 0, 40, 0));
                    remains = DeathSwap.getInstance().getGame().getCountdown();
                    return;
                } else {
                    game.getInGamePlayers().forEach(p -> {
        ...
        }.runTaskTimer(DeathSwap.getInstance(), 0, 20);
    }

    public void resetCountdown() {
        this.remains = DeathSwap.getInstance().getGame().getCountdown();
